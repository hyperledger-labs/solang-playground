...
use crate::sema::ast::{
    Builtin, CallArgs, Diagnostic, EventDecl, Expression, ExternalCallAccounts, Namespace, RetrieveType,
};
...
pub fn check_var_usage_expression(ns: &mut Namespace, left: &Expression, right: &Expression, symtable: &mut Symtable) {
    ...
    match &variable.usage_type {
            ...
            if !variable.assigned {
                if variable.ty.is_contract_storage() {
                    return Some(Diagnostic::error(
                        variable.id.loc,
                        format!("storage reference '{}' must be assigned a value", variable.id.name),
                    ));
                } else {
                    return Some(Diagnostic::warning(
                        variable.id.loc,
                        format!("return variable '{}' has never been assigned", variable.id.name),
                    ));
                }
            }
            None
        ...
        VariableUsage::DestructureVariable => {
            if !variable.read {
                return Some(Diagnostic::warning(
                    variable.id.loc,
                    format!("destructure variable '{}' has never been used", variable.id.name),
                ));
            }
        ...
        VariableUsage::TryCatchReturns => {
            if !variable.read {
                return Some(Diagnostic::warning(
                    variable.id.loc,
                    format!("try-catch returns variable '{}' has never been read", variable.id.name),
                ));
            }
        ...
        VariableUsage::TryCatchErrorBytes => {
            if !variable.read {
                return Some(Diagnostic::warning(
                    variable.id.loc,
                    format!("try-catch error bytes '{}' has never been used", variable.id.name),
                ));
            }
        ...
        VariableUsage::TryCatchErrorString => {
            if !variable.read {
                return Some(Diagnostic::warning(
                    variable.id.loc,
                    format!("try-catch error string '{}' has never been used", variable.id.name),
                ));
            }
        ...
        VariableUsage::YulLocalVariable => {
            ...
            if !variable.read && !has_value {
                return Some(Diagnostic::warning(
                    variable.id.loc,
                    format!("yul variable '{}' has never been read or assigned", variable.id.name),
                ));
...
fn emit_warning_contract_variables(variable: &ast::Variable) -> Option<Diagnostic> {
    if variable.assigned && !variable.read {
        return Some(Diagnostic::warning(
            variable.loc,
            format!("storage variable '{}' has been assigned, but never read", variable.name),
        ));
    ...
    for event_no in 0..ns.events.len() {
        ...
        if let Some(contract_no) = event.contract {
            ...
            for base_no in ns.contract_bases(contract_no) {
                ...
                if let Some(ast::Symbol::Event(events)) =
                    ns.variable_symbols
                        .get(&(base_file_no, Some(base_no), event.id.name.to_owned()))
                {
                    shadowing_events(event_no, event, &mut shadows, events, ns);
                }
            }

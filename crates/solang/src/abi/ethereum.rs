    ...
    ns.contracts[contract_no]
        ...
        .map(|func| ABI {
            ...
            inputs: if func.ty == pt::FunctionTy::Function || func.ty == pt::FunctionTy::Constructor {
                Some(func.params.iter().map(|p| parameter_to_abi(p, ns)).collect())
            } else {
                None
            },
            outputs: if func.ty == pt::FunctionTy::Function {
                Some(func.returns.iter().map(|p| parameter_to_abi(p, ns)).collect())
            } else {
                None
            },
            anonymous: None,
        })
        .chain(ns.contracts[contract_no].emits_events.iter().map(|event_no| {
            let event = &ns.events[*event_no];

            ABI {
                name: event.id.name.to_owned(),
                mutability: String::new(),
                inputs: Some(event.fields.iter().map(|p| parameter_to_abi(p, ns)).collect()),
                outputs: None,
                ty: "event".to_owned(),
